...
public class TicTacToeBoardUI extends GameBoardUI
{
    ...
    public void paint(java.awt.Graphics g)
    {
        ...
        {
            g.drawLine( 0, i*getHeight()/numRows, getWidth(), i*getHeight()/numRows ) ;
        }
        for( int i=1; i<numCols; i++ )
        {
            g.drawLine( i*getWidth()/numCols, 0, i*getWidth()/numCols, getWidth() ) ;
        }
        ...
        for( int row=0; row<numRows; row++ )
        {
            for( int col=0; col<numCols; col++ )
            {
                ...
                drawCell( tempPosition.row, tempPosition.col, tempPosition.perspective, g ) ;
            }
        }
    }
